...
const Coverage = () => {
  ...
  function handleSearch() {
    const searchArea = searchRef.current.value.trim().toLowerCase();
    const newArea = area.filter((ele) =>
  ...
  return (
    <div>
      ...
      <div
        className="my-4 shadow p-4 rounded-2xl bg-gray-200 min-h-[50vh]"
        data-aos="fade-up"
      >
        <MapContainer
          center={position}
        ...
        >
          ...
          {area.map((ele, index) => {
            const center = [ele.latitude, ele.longitude];
            return (
              <Marker position={center} key={index}>
                <Popup>
                  <div className="p-3">
